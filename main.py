...
def authorize_account():
    ...
    try:
        ...
        if not phone.startswith('+'):
            print("❌ Номер должен начинаться с '+' (например, +1234567890)")
            return
        
        # Спрашиваем про имя сессии
        session_choice = input("🔧 Хотите указать имя сессии? (y/n, по умолчанию автоматически): ").strip().lower()
        
        args = ["--phone", phone]
        
        if session_choice in ['y', 'yes', 'да']:
            session_name = input("📝 Введите имя сессии (например, account_7): ").strip()
            if session_name:
                args.extend(["--session", session_name])
        
        print(f"\n🚀 Запуск авторизации для {phone}...")
        print("📋 После запуска введите код из Telegram когда будет запрошен")
        print()
        
        # Запускаем скрипт авторизации
        run_script("scripts/auth_manager.py", args)
